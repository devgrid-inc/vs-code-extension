...
export class DevGridTreeDataProvider implements vscode.TreeDataProvider<DevGridTreeItem> {
  ...
  constructor(serviceContainer: ServiceContainer, authService: AuthService) {
  ...
  async refresh(): Promise<void> {
    ...
    try {
      ...
      if (!hasYaml) {
        this.errorMessage =
          'No valid devgrid.yml found. Set up your configuration to see insights.';
        this.insights = undefined;
  ...
  getDashboardUrl(): string | undefined {
    const urlTemplate =
      this.context?.config?.endpoints?.dashboardUrl || this.insights?.repository?.url;
    ...
    const valueMap = {
      repositoryId: this.insights?.repository?.id ?? this.context?.identifiers.repositoryId,
      repositorySlug: this.insights?.repository?.slug,
      componentId: this.insights?.component?.id ?? this.context?.identifiers.componentId,
      componentSlug: this.insights?.component?.slug ?? this.context?.identifiers.componentSlug,
      applicationId: this.insights?.application?.id ?? this.context?.identifiers.applicationId,
      applicationSlug:
        this.insights?.application?.slug ?? this.context?.identifiers.applicationSlug,
    };
  ...
  getChildren(element?: DevGridTreeItem): vscode.ProviderResult<DevGridTreeItem[]> {
    if (!element) {
  ...
  private getRootItems(): DevGridTreeItem[] {
    ...
    if (!this.insights) {
      return [DevGridTreeItem.info('No DevGrid insights available.', 'question')];
    }
    ...
    return [
      ...
      DevGridTreeItem.section('Vulnerabilities', 'section:vulnerabilities', 'shield'),
      DevGridTreeItem.section('Incidents', 'section:incidents', 'warning'),
      DevGridTreeItem.section('Dependencies', 'section:dependencies', 'versions'),
    ];
  }
  ...
  private getRepositoryItems(): DevGridTreeItem[] {
    ...
    const items: DevGridTreeItem[] = [
      ...
      repo.slug ? DevGridTreeItem.detail(`Slug: ${repo.slug}`, 'symbol-method') : undefined,
      repo.id ? DevGridTreeItem.detail(`ID: ${repo.id}`, 'symbol-key') : undefined,
      repo.description ? DevGridTreeItem.detail(repo.description, 'note') : undefined,
    ];

    // Add linkage status indicator
    if (linkageStatus) {
      const linkageIcon =
        linkageStatus.repoComponentLinked === true
          ? 'check'
  ...
  private getComponentItems(): DevGridTreeItem[] {
    ...
    return [
      ...
      component.slug ? DevGridTreeItem.detail(`Slug: ${component.slug}`, 'tag') : undefined,
      component.id ? DevGridTreeItem.detail(`ID: ${component.id}`, 'symbol-key') : undefined,
      component.description ? DevGridTreeItem.detail(component.description, 'note') : undefined,
      componentDevGridUrl
  ...
  private getApplicationItems(): DevGridTreeItem[] {
    ...
    return [
      DevGridTreeItem.detail(`Name: ${application.name ?? 'Unknown'}`, 'layers'),
      application.slug ? DevGridTreeItem.detail(`Slug: ${application.slug}`, 'tag') : undefined,
      application.id ? DevGridTreeItem.detail(`ID: ${application.id}`, 'symbol-key') : undefined,
      application.description ? DevGridTreeItem.detail(application.description, 'note') : undefined,
...
export class DevGridTreeItem extends vscode.TreeItem {
  ...
  static section(label: string, contextValue: string, iconId: string): DevGridTreeItem {
  ...
  static detail(label: string, iconId?: string): DevGridTreeItem {
    const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'detail');
    if (iconId) {
      item.iconPath = new vscode.ThemeIcon(iconId);
    }
    return item;
  ...
  static link(label: string, url: string, iconId?: string): DevGridTreeItem {
    const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'link');
    item.command = {
      command: 'vscode.open',
  ...
  static empty(label: string): DevGridTreeItem {
    const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'empty');
    item.iconPath = new vscode.ThemeIcon('info');
    return item;
  }

  static info(label: string, iconId: string): DevGridTreeItem {
    const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'info');
    item.iconPath = new vscode.ThemeIcon(iconId);
    return item;
  }
...
function createVulnerabilityItem(vulnerability: DevGridVulnerability): DevGridTreeItem {
  ...
  const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'vulnerability');
  ...
  item.tooltip = buildTooltip([
    ...
    vulnerability.versionRange ? `Range: ${vulnerability.versionRange}` : undefined,
    vulnerability.publishedAt ? `Published: ${vulnerability.publishedAt}` : undefined,
  ]);
...
function createIncidentItem(incident: DevGridIncident): DevGridTreeItem {
  ...
  const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'incident');
  const descriptionParts = [
...
function createDependencyItem(dependency: DevGridDependency): DevGridTreeItem {
  ...
  const item = new DevGridTreeItem(label, vscode.TreeItemCollapsibleState.None, 'dependency');
  const descriptionParts = [
...
function renderTemplate(template: string, context: Record<string, string | undefined>): string {
  return template.replace(/\{([^}]+)\}/g, (_match, key: string) => context[key] ?? '');
}
