...
export class DevGridAuthProvider implements vscode.AuthenticationProvider {
  ...
  private combineScopes(base: string | readonly string[], extra: readonly string[]): string[] {
    const baseArray =
      typeof base === 'string' ? base.split(/\s+/).filter(Boolean) : Array.from(base);
  ...
  private buildAccountInfo(token: TokenResponse): vscode.AuthenticationSessionAccountInformation {
    ...
    const id = decoded?.sub ?? decoded?.email ?? decoded?.preferred_username ?? randomUUID();
    const label = decoded?.name ?? decoded?.email ?? decoded?.nickname ?? 'DevGrid Account';
          ...
          response.on('end', () => {
            ...
            if (response.statusCode && response.statusCode >= 200 && response.statusCode < 300) {
              try {
                ...
                reject(
                  new Error(`Invalid response from auth service: ${(error as Error).message}`)
                );
              }
            } else {
              try {
                ...
                reject(
                  new DeviceFlowError(parsed.error ?? 'error', parsed.error_description ?? raw)
                );
              } catch {
                reject(
                  new Error(`Auth service error (${response.statusCode ?? 'unknown'}): ${raw}`)
                );
              }
            }
          });
